...
# 📅 CALENDARIO ACTUALIZADO DE LOTERÍAS (SIN ASTRO LUNA NI ASTRO SOL)
...
def obtener_todas_loterias():
    """Obtiene resultados de todas las loterías (SIN ASTRO LUNA NI ASTRO SOL)"""
    loterias_urls = {
...
def analizar_patrones(nombre_archivo):
    ...
    for i, (num, cnt) in enumerate(top.items(), 1):
        ...
        print(f" {i:2d}. {num.zfill(4)} │ {cnt:3d}x │ Lotería: {loteria_top}")
...
def analizar_todos_numeros(nombre_archivo):
    ...
    for idx, (numero, freq) in enumerate(top_20.items(), 1):
        ...
        print(f" {idx:2d}. {numero.zfill(4)}: {freq:3d}x │ Lotería: {loteria_top:12s}({loteria_count:2d})")
