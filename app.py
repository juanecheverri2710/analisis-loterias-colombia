...
def ejecutar_scraping_y_analisis():
    ...
    try:
        ...
        with lock:
            ...
            print(f"\n✅ ¡Análisis completado!")
            print(f"📊 Predicciones generadas: {len(predicciones_diarias)}")
            print(f"📌 Datos del último sorteo: {len(datos_ultimo_sorteo)}")
            print(f"🔮 Números especiales analizados: {len(analisis_numeros_especiales)}")
            print(f"📈 Caracteres de análisis: {len(analisis_texto)}")
...
# ✅ CORRECCIÓN: Ruta /start-analysis (era 404)
@app.route("/start-analysis", methods=["POST"])
def start_analysis():
    """Inicia el análisis en un hilo separado"""
    print("🔄 Endpoint /start-analysis llamado")
    thread = threading.Thread(target=ejecutar_scraping_y_analisis, daemon=True)
    thread.start()
    return jsonify({"status": "success", "message": "Análisis iniciado."})

# ✅ CORRECCIÓN: Ruta /get-results
@app.route("/get-results", methods=["GET"])
def get_results():
    """Devuelve los resultados del análisis"""
    global analisis_texto
    return jsonify({
        "status": "success" if analisis_texto else "pending",
        "resultados": analisis_texto if analisis_texto else "Análisis en progreso..."
    })

# ✅ CORRECCIÓN: Ruta /get-sorteos
@app.route("/get-sorteos", methods=["GET"])
def get_sorteos():
    """Devuelve datos del último sorteo de cada lotería"""
    return jsonify({"status": "success", "resultados": datos_ultimo_sorteo})

# ✅ CORRECCIÓN: Ruta /get-predicciones
@app.route("/get-predicciones", methods=["GET"])
def get_predicciones():
    """Devuelve predicciones de números ganadores para las loterías que juegan HOY"""
    return jsonify({"status": "success", "predicciones": predicciones_diarias})

# ✅ CORRECCIÓN: Ruta /get-analisis-numeros
...
# ✅ CORRECCIÓN: Ruta /get-calendario
...
def get_calendario():
    ...
    return jsonify({"status": "success", "calendario": calendario})

# ✅ CORRECCIÓN: Ruta /static
...
def obtener_historico():
    ...
    try:
        ...
        historico = obtener_todas_loterias()
